...
Recently I pushed the project in a direction that makes it easier to operate and demo as a real firmware system instead of just a watch UI demo.
...
- README cleanup so the repo reads more like a real open-source firmware project

The result is a project that is easier to test, script, and explain.
...
The recent protocol work matters for the same reason: it gives the firmware a cleaner operator surface.
...
The repository now looks much more like a complete open-source embedded platform: firmware, bootloader, flashing path, host tooling, images, and documentation all live together.
